...
BENCHMARK_STATE = {
    ...
    "logs": [],
    # Comparison baselines: Published reference scores from model evaluation papers.
    # "deepthink_aios" values are ESTIMATED targets based on the multi-agent
    # architecture design, NOT measured results from this system. They serve as
    # aspirational reference points for the benchmark UI dashboard.
...
# If HuggingFace datasets fail to download, the runner falls back to these
# PLACEHOLDER problems. Each entry uses a single repeated prompt as a skeleton;
# actual benchmark evaluation requires downloading the real datasets.
MOCK_PROBLEMS = {
    "HumanEval": [
