...
def create_jwt_token(user_id, extra_claims=None):
    ...
    try:
        ...
        # Fallback: hmac.new may not exist on some restricted builds
        import hashlib as _hl
        signature = _hl.sha256(JWT_SECRET.encode() + signing_input.encode()).digest()
...
if AUTH_ENABLED:
    print("[AUTH] Authentication: ENABLED — JWT tokens required for API access.")
else:
    print("[AUTH] Authentication: DISABLED — Open access (local development mode).")
