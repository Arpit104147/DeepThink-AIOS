...
def get_install_instructions():
    ...
    if not tools.get('iverilog') or not tools.get('vvp'):
        instructions.append(
            ...
            "   Windows:       choco install iverilog  OR  download from http://bleyer.org/icarus/\n"
    ...
    if not tools.get('yosys'):
        instructions.append(
            ...
            "   Windows:       Download from https://github.com/YosysHQ/oss-cad-suite-build/releases\n"
    ...
    if not tools.get('ngspice'):
        instructions.append(
            ...
            "   Windows:       choco install ngspice  OR  download from https://ngspice.sourceforge.io/download.html\n"
...
def install_tools(tools_to_install=None):
    ...
    if apt_packages:
        ...
        if shutil.which('apt-get'):
            ...
            # Windows: Try Chocolatey package manager
            for pkg in apt_packages:
                try:
                    result = subprocess.run(
                    ...
                    if result.returncode == 0:
                        results[pkg] = "✅ Installed via Chocolatey"
                    else:
                        results[pkg] = f"❌ choco install failed: {result.stderr[:200]}"
                except Exception as e:
                    results[pkg] = f"❌ Chocolatey error: {str(e)}"
        elif shutil.which('winget'):
            # Windows: Try winget
            for pkg in apt_packages:
                results[pkg] = f"❌ winget install not automated for {pkg}. Install manually."
        else:
            for pkg in apt_packages:
                results[pkg] = "❌ No package manager found (apt-get/choco/winget). Install manually."
