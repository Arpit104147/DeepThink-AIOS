...
ALLOWED_MODULES = {
    ...
    # from this whitelist. While 'scapy' is in requirements.txt for the
    # security module's packet analysis, the restricted sandbox forbids all
    # network I/O. Use SandboxDataHelper for deterministic mock data instead.
}
...
class SandboxDataHelper:
    ...
    def get_stock_data(symbol, period="1y"):
        try:
            ...
            # Use symbol-based seed so different tickers produce different data
    ...
    def get_weather_data(city, days=7):
        try:
            ...
            dates = [datetime.now() + timedelta(days=i) for i in range(days)]
            # Use city-based seed so different cities produce different data
            np.random.seed(hash(city.upper()) % (2**31))
...
"""
    ...
    def _apply_seccomp_preexec():
        ...
        """
        if os.name == 'nt':
            return None  # resource module not available on Windows
        try:
            import resource
        except ImportError:
    ...
    def _run_with_kill(self, cmd, timeout, cwd=None, env=None, use_isolation=False):
        ...
        if os.name == 'posix':
            ...
            if use_isolation:
                preexec = self._apply_seccomp_preexec()
                if preexec is not None:
                    popen_kwargs['preexec_fn'] = preexec
        elif os.name == 'nt':
            # On Windows, CREATE_NEW_PROCESS_GROUP enables clean process tree
            # termination via taskkill /T. This is the Windows equivalent of
        ...
        try:
            ...
            if os.name == 'posix':
                ...
                # (the process and all its children). Plain proc.kill() only
                # kills the immediate process, leaving grandchildren orphaned.
                try:
                    subprocess.run(
