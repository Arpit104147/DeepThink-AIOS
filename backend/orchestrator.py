    ...
    class MockVirtualMemory:
        def __init__(self):
            ...
            if sys.platform == 'win32':
                try:
                    # Use ctypes to call Windows GlobalMemoryStatusEx
                    import ctypes
...
class AgentOrchestrator:
    def __init__(self, cancel_event=None):
        ...
        if has_dgpu:
            ...
            self.kaggle_hotswap_mode = False
            self.ram_safety_gb = round(self.total_ram_gb * 0.10, 1)  # 10% RAM safety
            print(f"[INFO] NORMAL MODE: iGPU/CPU detected, RAM={self.total_ram_gb:.0f}GB")
            print(f"[INFO] NORMAL: Sequential single-model loading (max context per model)")
            print(f"[INFO] NORMAL: RAM reserve={self.ram_safety_gb:.1f} GB")
        ...
        if self.dual_gpu_pipeline and has_dgpu:
            ...
            if has_dgpu:
                ...
                self.max_auto_ctx = min(32768, max(4096, int((kv_budget_gb * 1024) / 0.13)))
                self.max_auto_ctx = (self.max_auto_ctx // 1024) * 1024
                print(f"[INFO] NORMAL: Auto-context ceiling = {self.max_auto_ctx} tokens "
        ...
        print(f"[INFO] DMA: Set default context length to {self.context_length} tokens")
        print(f"[INFO] DMA: Detected {self.total_ram_gb:.0f} GB RAM -> "
    ...
    def _get_sysfs_gpu_vram(self):
        """Read GPU VRAM via platform-specific methods.
        Linux: AMD/Intel GPU VRAM via sysfs /sys/class/drm/.
        Windows: NVIDIA GPU VRAM via NVML ctypes (if available).
        Returns list of (free_gb, total_gb, card_name).
        """
        results = []
        if os.name == 'posix':
            import glob
            ...
            # Attempt NVIDIA NVML on Windows for non-CUDA GPU memory info
            try:
                import ctypes
                ...
                for i in range(device_count.value):
                    ...
                    results.append((free_gb, total_gb, f"GPU-{i}"))
                nvml.nvmlShutdown()
            except Exception:
